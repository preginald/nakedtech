...
- Forms commit `e7126d27f47ec7a64091a978cbe9916b9337974f` closes DATA-03 by applying shared state/origin/respondent-auth checks to draft reads, writes and deletes, enforcing a 256 KiB JSON-object ceiling, accepting only schema-declared editable draft fields, excluding secret/non-editable fields and applying a 120 requests/minute per-client/per-form draft budget. Ruff, 487 backend tests, 103 frontend tests, both production builds, pull-request CI, post-merge CI and deployment passed. Production matched the merged SHA with Forms and MCP active; a no-token/no-body probe proved a blocked origin receives `403` while the approved Naked Tech origin receives only an empty payload, without creating or reading a customer draft.
