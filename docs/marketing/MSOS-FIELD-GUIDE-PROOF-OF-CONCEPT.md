...
Mobile navigation visual refinement: the enhanced bar spans the viewport beneath the header with square corners and only a bottom divider. Its text aligns with the article column; the expanded menu retains inset rounded corners. Keyboard focus uses an inset outline.
